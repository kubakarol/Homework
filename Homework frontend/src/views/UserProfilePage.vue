        ...
        <ion-item v-for="(post, postIndex) in userPosts" :key="post.id">
          <ion-label>
            ...
            <div v-else>
              ...
              <ion-button v-if="post.userId === userId" @click="deletePost(post.id, postIndex)"><i class="bi bi-trash"></i></ion-button>
            </div>
            <h3>Comments</h3>
            <ion-list>
              <ion-item v-for="(comment, commentIndex) in post.comments" :key="comment.id">
                <ion-label>
                  ...
                  <div v-else>
                    ...
                    <ion-button v-if="comment.userId === userId" @click="editComment(comment, postIndex, commentIndex)" class="edit-button"><i class="bi bi-pencil-square"></i></ion-button>
...
export default {
  ...
  methods: {
    async fetchUserPosts() {
      ...
      try {
        const response = await axios.get(`https://localhost:7195/api/Post/getByUser/${userId}`);
